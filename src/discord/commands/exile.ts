...
import { generateMessage, MessageColors, MessageTitles, noLinkedAccounts } from '@/discord/messages';
import db from '@/db';
import { Client } from 'bloxy';
import { findAssociatedAccount } from '@/discord/util';

export async function exileCommand(interaction: APIChatInputApplicationCommandInteraction) {
    ...
    try {
        ...
        if (!rolePermissions.permissions.groupMembershipPermissions.inviteMembers) return generateMessage({ responseType: InteractionResponseType.ChannelMessageWithSource, title: MessageTitles.NoPermission, flags: MessageFlags.Ephemeral });
    } catch (err: any) {
        console.log(err)
        return generateMessage({ responseType: InteractionResponseType.ChannelMessageWithSource, title: MessageTitles.UnableExile, color: MessageColors.Red });
    };
            ...
            components: [
                ...
                },
                {
                    ...
                    components: [{
                        ...
                        label: 'Reason',
                        style: 2,
                        placeholder: "He's taking up space!",
                        max_length: 1024,
                        required: false
                    }]
                }
